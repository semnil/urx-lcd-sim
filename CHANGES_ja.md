...
- すべてのリダクションバーを、実機の COMP 画面のバーと同じ目盛り (18 dB で全長の 11/21、50 dB で全長) で描く。GATE・DUCKER・SSMCS の画面、INS FX の Compander、M.B.Comp の帯、チャンネルビューの COMP ブロックが対象。COMP ブロックの入力レベルの棒とスレッショルドの縦棒は、同じ目盛りを 0 dB から下向きに当てて描く。
