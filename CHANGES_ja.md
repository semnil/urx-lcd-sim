...
- すべてのリダクションバーを実機の目盛り (18 dB で全長の 11/21、36 dB で 11/14、60 dB で全長) で描く。GATE・DUCKER・COMP・SSMCS の画面、INS FX の Compander、M.B.Comp の帯、チャンネルビューの COMP ブロックが対象。
- すべてのレベルメーターを、同じ目盛りを 0 dB から下向きに当てて点ける (-18 dB で全長の 10/21、-36 dB で 3/14、-60 dB 以下で空)。これまでは -60〜0 dB を直線で点けていた。チャンネルビューの COMP ブロックの入力レベルの棒とスレッショルドの縦棒も同じ目盛りを使う。
